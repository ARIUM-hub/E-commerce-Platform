...
async function verifyBackupArchive({ archivePath, checksumPath, workingDir }) {
  const checksumLine = (await fsp.readFile(checksumPath, "utf8")).trim();
  const checksumMatch = /^([a-f0-9]{64})\s{2}(.+)$/.exec(checksumLine);
  if (!checksumMatch || checksumMatch[2] !== path.basename(archivePath)) {
...
function assertArchiveInBackupDir(archivePath, backupDir) {
  const resolvedBackupDir = path.resolve(backupDir);
  const resolvedArchivePath = path.resolve(archivePath);
  if (path.dirname(resolvedArchivePath) !== resolvedBackupDir) {
    throw new Error("Backup archive must be inside the configured backup directory");
  }
  if (!backupArchivePattern.test(path.basename(resolvedArchivePath))) {
    throw new Error("Backup archive filename is invalid");
  }
  return resolvedArchivePath;
}

function getBackupServiceVersion(archivePath) {
  const match = /-([a-f0-9]{7,40})\.db\.gz$/.exec(path.basename(archivePath));
  if (!match) throw new Error("Backup archive filename is invalid");
  return match[1];
}
...
async function restoreDatabaseBackup({
  ...
  const resolvedArchivePath = assertArchiveInBackupDir(archivePath, backupDir);
  const checksumPath = `${resolvedArchivePath}.sha256`;
  const resolvedTargetPath = path.resolve(targetPath);
  await fsp.stat(resolvedTargetPath);

  const restoreTempPath = path.resolve(
    path.dirname(resolvedTargetPath),
    `.${path.basename(resolvedTargetPath)}.${randomUUID()}.restore.db`
  );
  const serviceVersion = getBackupServiceVersion(resolvedArchivePath);
  let targetMoved = false;
  const rollbackBase = `${resolvedTargetPath}.rollback-${formatBackupTimestamp(now())}`;
  const movedCompanions = [];
  let preRestoreBackup;
  try {
    const checksumLine = (await fsp.readFile(checksumPath, "utf8")).trim();
    const checksumMatch = /^([a-f0-9]{64})\s{2}(.+)$/.exec(checksumLine);
    if (!checksumMatch || checksumMatch[2] !== path.basename(resolvedArchivePath)) {
      throw new Error("Backup checksum file is invalid");
    }
    const actualSha256 = await calculateFileSha256(resolvedArchivePath);
    if (!timingSafeEqual(
      Buffer.from(checksumMatch[1], "hex"),
    ...
    await pipeline(
      ...
      createGunzip(),
      fs.createWriteStream(restoreTempPath, { flags: "wx" })
    );
    const restoredDb = createDatabase(restoreTempPath);
    let integrity;
    try {
      integrity = readIntegrityResult(restoredDb);
    } finally {
      restoredDb.close();
    }
    if (integrity !== "ok") {
    ...
    await fsp.rename(resolvedTargetPath, rollbackBase);
    targetMoved = true;
    for (const suffix of ["-wal", "-shm"]) {
      if (await renameIfPresent(`${resolvedTargetPath}${suffix}`, `${rollbackBase}${suffix}`)) {
        movedCompanions.push(suffix);
      }
    }
    await fsp.rename(restoreTempPath, resolvedTargetPath);
    await removeDatabaseArtifacts(rollbackBase);
    return {
      targetPath: resolvedTargetPath,
      archivePath: resolvedArchivePath,
      preRestoreArchivePath: preRestoreBackup.archivePath,
      integrity
    };
  } catch (error) {
    if (targetMoved) {
      await removeDatabaseArtifacts(resolvedTargetPath);
      await fsp.rename(rollbackBase, resolvedTargetPath);
      for (const suffix of movedCompanions) {
        await renameIfPresent(`${rollbackBase}${suffix}`, `${resolvedTargetPath}${suffix}`);
      }
    }
    throw error;
  } finally {
    await removeDatabaseArtifacts(restoreTempPath);
  }
}
