...
function ensureCart(db, { userId, sessionId }) {
  ...
  db.prepare(`
    INSERT INTO carts (id, owner_type, user_id, session_id, coupon_code, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `).run(cartId, ownerType, userId || null, sessionId || null, null, timestamp, timestamp);
...
function setCartCouponCode(db, cartId, couponCode) {
  db.prepare("UPDATE carts SET coupon_code = ?, updated_at = ? WHERE id = ?")
    .run(couponCode || null, nowIso(), cartId);
}
