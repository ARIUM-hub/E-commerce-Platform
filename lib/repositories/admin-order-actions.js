...
function executeIdempotentAction(db, input) {
  ...
  const transaction = db.transaction(() => {
    ...
    if (result.validationError) {
      const rollbackError = new Error(result.validationError.message);
      rollbackError.adminActionValidationError = result.validationError;
      throw rollbackError;
    }
    const afterStatus = result.afterStatus
      || result.review?.status
      || result.order?.status
      || result.returnRequest?.status
      || input.beforeStatus;
  ...
  try {
    ...
    if (error.adminActionValidationError) {
      return { validationError: error.adminActionValidationError };
    }
    throw error;
  }
}
