...
function findSupportTicketByNumber(db, ticketNumber, options = {}) {
  const row = db.prepare("SELECT id FROM support_tickets WHERE ticket_number = ? LIMIT 1")
    .get(String(ticketNumber || "").trim());
  return row ? findSupportTicketById(db, row.id, options) : null;
}

function listAdminSupportTickets(db, filters = {}) {
  const tickets = db.prepare(`
    SELECT * FROM support_tickets
    ORDER BY datetime(COALESCE(last_message_at, created_at)) DESC, rowid DESC
  `).all().map(mapSupportTicketRow);
  ...
  const filtered = tickets.filter((ticket) => {
    const searchText = `${ticket.ticketNumber} ${ticket.name} ${ticket.contact} ${ticket.orderId} ${ticket.message}`.toLowerCase();
    return (!status || ticket.status === status)
      && (!priority || ticket.priority === priority)
      && (!topic || ticket.topic === topic)
      && (!assignee
        || (assignee === "unassigned" ? !ticket.assignedAdminUserId : ticket.assignedAdminUserId === assignee))
      && (!query || searchText.includes(query));
  });
  const countStatus = (value) => tickets.filter((ticket) => ticket.status === value).length;
  return {
    tickets: filtered,
    summary: {
      open: countStatus("open"),
      inProgress: countStatus("in_progress"),
      waitingCustomer: countStatus("waiting_customer"),
