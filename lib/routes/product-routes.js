...
function parseProductReviewHelpfulPath(pathname) {
  const match = pathname.match(/^\/api\/products\/([^/]+)\/reviews\/([^/]+)\/helpful$/);
  return match
    ? {
...
function registerProductRoutes(router, services) {
  ...
  router.add("POST", /^\/api\/products\/[^/]+\/reviews\/[^/]+\/helpful$/, async ({ request, requestUrl, response, sendJson, sendError }) => {
    const pathValues = parseProductReviewHelpfulPath(requestUrl.pathname);
    const activeCart = await services.readActiveCart(request, { createAnonymousSession: true });
    const owner = {
      userId: activeCart.user?.id || null,
      sessionId: activeCart.user ? null : activeCart.sessionId
    ...
    const result = services.withDatabase((db) => {
      const product = services.findProductById(db, pathValues.productId);
      if (!product) {
        return { productNotFound: true };
      }

      const marked = services.markProductReviewHelpful(db, pathValues.productId, pathValues.reviewId, owner);
      if (marked.reviewNotFound) {
        return { reviewNotFound: true };
      }
      if (marked.validationError) {
        return marked;
      }

      const reviews = services.listProductReviews(db, pathValues.productId);
      return {
        ok: true,
    ...
    if (result.reviewNotFound) {
      sendError(response, 404, "PRODUCT_REVIEW_NOT_FOUND", "Product review was not found.");
      return;
    }
